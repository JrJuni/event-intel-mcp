...
### #16 비용 최적화 6-레버 — 🚧 ①④⑤ 착수 (P1, 사용자 지시 2026-06-11 → phase plan `config-zero-mossy-toucan.md` v1)
...
**이번 phase 범위 (2026-06-11 사용자 확정)**: ①④⑤ + 홈페이지 크롤 lane. **검증은 skeleton+mock only — 라이브 런 금지.**

1. **① CSV 직변환 short-circuit — 🚧 이번 phase**: 컬럼 매핑만으로 candidate 직생성 → CSV run extraction $4.35→$0 + head-truncation 구조 결함 동시 제거.
...
4. **④ 스테이지별 모델 right-sizing — 🚧 이번 phase**: triage/fit→Haiku급(PROVISIONAL, 품질 라이브 검증 차후), rationale은 Sonnet 유지. ledger에 blended cost 환산.
5. **⑤ LLM 결과 캐시 — 🚧 이번 phase**: `(chunk_sha, prompt_sha, model, lang)` 키 영속 캐시 — 재실행 재과금 제거. "75분 빌드가 chunk 39에서 전사" 손실의 근본 처방(retry-playbook §3.6).
6. **⑥ Brave rescue rung — ⏸️ deferred**: 6월 Brave 쿼터 소진이라 라이브 검증 불가. (단, 이번 phase의 **홈페이지 크롤 lane이 쿼리 ~150→~30을 먼저 달성** — Brave rescue의 필요 자체가 줄어들 수 있음, 재평가.)
