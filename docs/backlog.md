...
### #16 비용 최적화 6-레버 — run당 비용 $2 → $0.3~0.4 (P1, 사용자 지시 2026-06-11)

**배경 (D3 실측 근거)**: 2026-06-11 D3 매트릭스 실측에서 run당 비용 구조 확정 — **검색이 지배**(Brave 1 cold run ≈ 150 쿼리 ≈ $0.75, 무료+유료 양 쿼터 당일 소진), LLM Sonnet 환산 $0.18(p1)~$1.51(p7, 대형 HTML 43청크). 실지출은 extraction Sonnet 호출만(fit/triage는 OAuth 무료 lane). 사용자: "이번 시퀀스 끝나면 플랜 짜고 시작하자" — **D3 종료 후 별도 phase plan으로 착수**. 메모리 [[june-2026-structure-focus]].

우선순위순 6개 레버 (추정 효과: HTML cold run $2 → $0.3~0.4, CSV run < $0.2):

1. **CSV 직변환 short-circuit (최우선 — 비용 + 정확성 동시 해결)**: CSV 소스는 이미 구조화돼 있는데 현재 LLM extraction을 통과 — 컬럼 매핑만으로 candidate 직생성하면 p7급 extraction 비용(~$1.3)이 0이 되고, **p7에서 12/43 청크 head-truncation으로 gold 4사 전원이 추출 전 탈락하던 구조 결함도 함께 제거**(D3 p7 default tgt_cov 0/4 실측; fullx 변형으로 통제 검증).
2. **Anthropic prompt caching**: extraction 시스템 프롬프트 + capability 발췌가 호출마다 반복 입력 — cache_control 적용 시 캐시 입력 90% 할인.
3. **Batch API**: 벤치마크·비대화형 빌드는 지연 허용 — Anthropic Batch 50% 할인.
4. **스테이지별 모델 right-sizing**: extraction→Haiku, fit/triage→mini급, rationale만 Sonnet. D3 실측 mini 환산 ≈ Sonnet의 29%.
5. **LLM 결과 캐시**: `(chunk_hash, prompt_version)` 키 영속 캐시 — 재실행·resume 시 동일 청크 재과금 제거(현재 enrichment 캐시만 있고 extraction/fit 캐시 없음).
6. **Brave rescue rung**: ddgs 1차 → 결과 빈약한 회사만 Brave — 쿼리 ~150 → ~30 (~$0.75 → ~$0.15/run). `docs/retry-playbook.md` 사다리에 통합.
