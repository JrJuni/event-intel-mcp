...
- **Workspace & Source Library RAG (WSL) — 신규 제품 역량 ✅ 전체 완료 (2026-06-08~09, plan `snoopy-weaving-robin.md` 상단 v0.2, W0–W5 전 슬라이스 머지 #39~#44)**
  ...
  - **WSL 결과**: card collection(`product_{ws}`) ↔ raw source collection(`product_sources_{ws}`) 분리 완성. 사용자 작업폴더 PDF/MD/TXT/CSV → 증분 인덱싱 → 더 나은 카드 초안(`--from-workspace`) + S/A rationale provenance(report v4). **점수/티어 경로 불변**(structural + headline 테스트). 경로는 ResolvedPaths 단일 resolver. 다음 우선순위는 로드맵상 **Y1D(rerank/retrieval)** 측정 후 fix → holdout 전 threshold 재freeze(hard-gate).
